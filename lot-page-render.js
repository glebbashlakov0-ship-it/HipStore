(function () {
  ...
  // Preload slug→endTime lookup (generated by download-lots.js)
  var _lotEndTimes = null;
  function loadLotEndTimes() {
    if (_lotEndTimes) return Promise.resolve(_lotEndTimes);
    return loadJson(BASE_PATH + "data/lot-end-times.json")
      .then(function (data) { _lotEndTimes = data || {}; return _lotEndTimes; })
      .catch(function () { _lotEndTimes = {}; return _lotEndTimes; });
  }
  ...
  function formatBidHistoryAmount(value) {
  ...
  function getStatusLabel(item, strings) {
    ...
    if (status === "closed" || status === "sold" || status === "ended") return strings.bid.auctionEnded;
    if (status === "active") return strings.bid.timeRemaining;
  ...
  function resolveEndTime(status, endTime, slug) {
    if (String(status || '').toLowerCase() !== 'active') return endTime || '';
    // Try lookup from lot-end-times.json (generated by download-lots.js)
    if (_lotEndTimes && slug && _lotEndTimes[slug]) {
      var looked = _lotEndTimes[slug];
      var diff = new Date(looked).getTime() - Date.now();
      if (Number.isFinite(diff) && diff > 0) return looked;
    }
    // Real end_time still in future — use it
    var realDiff = new Date(endTime).getTime() - Date.now();
    if (Number.isFinite(realDiff) && realDiff > 0) return endTime;
  ...
  function normalizeLotTranslation(item, language) {
    ...
    if (!translation) {
      var resolved = resolveEndTime(item.status, item.end_time, item.slug);
      if (resolved !== item.end_time) return Object.assign({}, item, { end_time: resolved });
      return item;
    }
    ...
    return {
      ...
      end_time: resolveEndTime(item.status, item.end_time, item.slug),
      status: item.status,
      year: item.year,
  ...
  function normalizeFallbackLot(item) {
    ...
    return {
      ...
      end_time: resolveEndTime(item.status || "active", item.endTime || item.end_time || "", item.slug),
  ...
  function renderRelatedCard(item, strings) {
    ...
    var isEnded = ["closed", "sold", "ended"].indexOf(String(item.status || "").toLowerCase()) !== -1 || (String(item.status || "").toLowerCase() !== "active" && parts.ended);
  ...
  function getBidHistoryItems(bids, lot) {
    var safeBids = Array.isArray(bids) ? bids.slice() : [];
    if (!safeBids.length) {
      safeBids = buildFallbackBids(lot);
    }
  ...
  function maskBidderEmail(email) {
    var value = String(email || "private.bidder@example.com").trim();
    var match = value.match(/^([^@]+)@(.+)$/);
    if (!match) return "P***";
    var local = match[1];
    var domain = match[2];
    var firstChar = local ? local.charAt(0).toUpperCase() : "P";
    return firstChar + "***@" + domain;
  }

  function formatBidHistoryTime(value) {
    if (!value) return "";
  ...
  function renderBidHistoryItems(bids, lot) {
    var safeBids = getBidHistoryItems(bids, lot);

    return safeBids.map(function (bid, index) {
      var previous = safeBids[index + 1];
      var increase = previous ? Number(bid.amount || 0) - Number(previous.amount || 0) : Number(bid.amount || 0) - Number(lot.starting_bid || 0);
      var increaseLabel = (increase > 0 ? "+" : "") + formatBidHistoryAmount(increase);
      return (
        '<div class="flex items-start justify-between gap-4 py-3 border-b last:border-0">' +
        '<div class="flex-1 min-w-0">' +
        '<div class="flex items-center gap-2 mb-1"><span class="font-medium text-sm">' + escapeHtml(maskBidderEmail(bid.email || "Private Bidder")) + "</span></div>" +
        '<p class="text-xs text-muted-foreground mt-1">' + escapeHtml(formatBidHistoryTime(bid.created_at)) + "</p>" +
        "</div>" +
        '<div class="text-right flex-shrink-0">' +
        '<p class="font-semibold text-sm">' + formatBidHistoryAmount(bid.amount || 0) + "</p>" +
        '<p class="text-xs text-green-600 dark:text-green-400">' + escapeHtml(increaseLabel) + "</p>" +
        "</div>" +
        "</div>"
      );
    }).join("");
  ...
  function renderLot(main, lot, relatedLots, bids, strings) {
    ...
    main.innerHTML =
      ...
      '<div role="dialog" id="lot-bid-history-dialog" aria-modal="true" aria-labelledby="lot-bid-history-title" data-state="open" data-slot="dialog-content" class="bg-background fixed top-[50%] left-[50%] z-50 translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border shadow-lg duration-200 sm:max-w-lg max-w-md w-[95vw] sm:w-full max-h-[70vh] sm:max-h-[80vh] overflow-hidden flex flex-col p-4 sm:p-6" tabindex="-1" style="pointer-events:auto;">' +
      '<div data-slot="dialog-header" class="flex flex-col gap-2 text-center sm:text-left pb-3">' +
      '<h2 id="lot-bid-history-title" data-slot="dialog-title" class="font-semibold text-base sm:text-lg">' + escapeHtml(strings.productPage.bidHistory) + " (" + escapeHtml(String(getBidHistoryItems(bids, lot).length)) + " bids)</h2>" +
      "</div>" +
      '<div class="overflow-y-auto flex-1 -mx-4 px-4 sm:-mx-6 sm:px-6"><div class="space-y-3">' + renderBidHistoryItems(bids, lot) + "</div></div>" +
      '<button type="button" data-close-bid-history data-slot="dialog-close" class="ring-offset-background focus:ring-ring data-[state=open]:bg-accent data-[state=open]:text-muted-foreground absolute top-4 right-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*=\'size-\'])]:size-4" aria-label="Close"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg><span class="sr-only">Close</span></button>' +
      "</div></div>" +
  ...
  function wireBidHistoryModal(scope) {
    ...
    var dialog = modal.querySelector("#lot-bid-history-dialog");
